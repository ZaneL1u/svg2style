function addfun(str: string, array: any[]) {
    if (str && array) {
...
function getReg(str: string, reg: RegExp) {
...
//单个字符处理
function round_single(str: string) {
    let arr = str2arr(str, ".");
    let newarr = new Array();
...
//列表字符处理
function round_link(str: string) {
    let arr = str2arr(str, "-");
    let newarr = new Array();
    for (let i in arr) {
...
// svg转css（多色）
function svg2style(code: string, color?: string[]) {
    ...
    //path
    let reg =
        /path(\s{0,}fill\=\"(\#{0,1}[A-z0-9]*)?\")?\s{1,}d\=\"[A-z0-9\s\.\-\,]*\"(\s{0,}fill\=\"(\#{0,1}[A-z0-9]*)?\")?/g;
    let regd = /d\=\"([A-z0-9\s\.\-\,]*)\"/;
    let coded = "d";
    ...
    if (!path) {
        //polygon  尝试读取形状
        reg =
            /polygon(\s{0,}fill\=\"(\#{0,1}[A-z0-9]*)?\")?\s{1,}points\=\"[A-z0-9\s\.\-\,]*\"(\s{0,}fill\=\"(\#{0,1}[A-z0-9]*)?\")?/g;
    ...
    let css: string | undefined = "data:image/svg+xml,%3Csvg";
